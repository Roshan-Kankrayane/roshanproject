({
    ...
    save: function (component, event, helper, contactId, onsuccess, onerror) {
        ...
        if(!(component.get("v.isFTEDisabled") || objAccount.Number_of_Full_Time_Employees__c == component.get("v.previousFTE"))){
            objAccount.FTE_and_HQ_Updated_From__c = component.get("v.pageSourceName");
        }
        ...
        utils.execute("c.getRevStarAPIPrice", {"recordId": opportunity.Building__c, "quantity":opportunity.Interested_in_Number_of_Desks__c}, function(result){    
            revStarResult.RevStar_Price = parseFloat(result.RevStar_Price);
            revStarResult.Success = result.Success;
            revStarResult.PriceType = result.PriceType;
            revStarResult.Currency = result.Currency;
            utils.execute("c.saveRecords",{"records": lstRecordsToUpdate}, function(response){
                utils.execute("c.insertAddOpportunity", {"mapOpportunityData": opportunity,"mapRevstarResult":revStarResult}, function(result){
    ...
    initOpportunity: function(component, event, helper){ //Method to initialize new Opportunity object with prepopulated values.		
        ...
        } else if (journeyOpportunityData.account && journeyOpportunityData.account.Unomy_Company_Size_Formula__c != undefined) {
            opportunity["Number_of_Full_Time_Employees__c"] = parseInt(journeyOpportunityData.account.Unomy_Company_Size_Formula__c);
        ...
        } else if (journeyOpportunityData.contact && journeyOpportunityData.contact.Unomy_Company_Size_Formula__c != undefined) {
           opportunity["Number_of_Full_Time_Employees__c"] = parseInt(journeyOpportunityData.contact.Unomy_Company_Size_Formula__c);
        } else if(journeyOpportunityData.lead && (journeyOpportunityData.lead.Number_of_Full_Time_Employees__c != undefined || journeyOpportunityData.lead.Unomy_Company_Size_Formula__c != undefined)){
            if(journeyOpportunityData.lead.Unomy_Company_Size_Formula__c != undefined){                
                opportunity["Number_of_Full_Time_Employees__c"] = parseInt(journeyOpportunityData.lead.Unomy_Company_Size_Formula__c);                
            ...
            component.find("utils").execute("c.saveRecord", {"record": {"Id" : component.get("v.journey").Primary_Lead__c, "Account__c" : component.get("v.OrgRecord.Id"), "Number_of_Full_Time_Employees__c" : component.get("v.accountRec.Number_of_Full_Time_Employees__c"), "Interested_in_Number_of_Desks__c" : component.get("v.accountRec.Interested_in_Number_of_Desks__c")}}, function(response){
                component.find("utils").execute("c.convertLeadsToExisting", {"objLead" : {"Id" : component.get("v.journey").Primary_Lead__c, "Email" : component.get("v.journey").Primary_Lead__r.Email}}, function(response){
                    if(response && response != "null"){
                        ...
                        component.find("utils").execute("c.getJourneyOpportunityData", {"mapOpportunityData": opportunityData}, function(response){
                            let journeyOpportunityResult = JSON.parse(response);
                            if(journeyOpportunityResult && journeyOpportunityResult.account != null){
                              component.set("v.previousFTE", journeyOpportunityResult.account.Number_of_Full_Time_Employees__c);  
                            }
                            component.set("v.journeyOpportunityData", journeyOpportunityResult);
